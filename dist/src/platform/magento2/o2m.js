    ...
    const processCart = (result) => {
        ...
        api.cart.pull(null, cartId, null, isThisAuthOrder).then((serverItems) => {
            ...
            for (const clientItem of clientItems) {
                const serverItem = serverItems.find(itm => vsf_utilities_1.productsEquals(itm, clientItem));
                if (!serverItem) {
            ...
            for (const serverItem of serverItems) {
                if (serverItem) {
                    const clientItem = clientItems.find(itm => vsf_utilities_1.productsEquals(itm, serverItem));
                    if (!clientItem) {
